...
@test "Create node function no schema" {
    ...
    run_with_retry "dispatch get function node-hello-no-schema -o json | jq -r .status" "READY" 8 5
}
...
@test "Execute powershell function no schema" {
    run_with_retry "dispatch exec powershell-hello-no-schema --input='{\"name\": \"Jon\", \"place\": \"Winterfell\"}' --wait -o json | jq -r .output.myField" "Hello, Jon from Winterfell" 5 5
}
...
@test "Execute java with runtime deps" {
    run_with_retry "dispatch exec java-hello-with-deps --wait -o json | jq -r .output" "Hello, Someone from timezone UTC" 5 5
}
...
@test "Create java function with classes" {
    ...
    run_with_retry "dispatch get function java-hello-with-classes -o json | jq -r .status" "READY" 20 5
}

@test "Execute java with classes" {
    run_with_retry "dispatch exec java-hello-with-classes --input='{\"name\": \"Jon\", \"place\": \"Winterfell\"}' --wait -o json | jq -r .output.myField" "Hello, Jon from Winterfell" 5 5
}
...
@test "Create java function with spring support" {
    ...
    run_with_retry "dispatch get function spring-fn -o json | jq -r .status" "READY" 20 5
}

@test "Execute java with spring support" {
    run_with_retry "dispatch exec spring-fn --input='{\"name\":\"Jon\", \"place\":\"Winterfell\"}' --wait -o json | jq -r .output.myField" "Hello, Jon from Winterfell" 5 5
}
...
@test "Execute node function with schema" {
    run_with_retry "dispatch exec node-hello-with-schema --input='{\"name\": \"Jon\", \"place\": \"Winterfell\"}' --wait -o json | jq -r .output.myField" "Hello, Jon from Winterfell" 5 5
}
...
@test "Execute python function with runtime deps" {
    run_with_retry "dispatch exec http --wait -o json | jq -r .output.status" "200" 5 5
}
...
@test "Create python function with invalid handler" {
    ...
    run_with_retry "dispatch get function invalid -o json | jq -r .status" "ERROR" 5 5
...
EOF
    ...
    run_with_retry "dispatch get function logger -o json | jq -r .status" "READY" 6 5
}

@test "Execute python function with logging" {
    run_with_retry "dispatch exec logger --wait -o json | jq -r \".logs.stderr | .[0]\"" "this goes to stderr" 5 5
    run_with_retry "dispatch exec logger --wait -o json | jq -r \".logs.stdout | .[0]\"" "this goes to stdout" 5 5
}

@test "Execute function with a literal value payload" {
    run_with_retry "dispatch exec http --input='42' --wait -o json | jq -r .output.status" "200" 5 5
}
