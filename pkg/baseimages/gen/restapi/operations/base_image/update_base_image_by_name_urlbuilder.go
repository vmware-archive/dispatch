...
func (o *UpdateBaseImageByNameURL) Build() (*url.URL, error) {
	...
	var _path = "/{baseImageName}"

	baseImageName := o.BaseImageName
	if baseImageName != "" {
		_path = strings.Replace(_path, "{baseImageName}", baseImageName, -1)
	} else {
		return nil, errors.New("BaseImageName is required on UpdateBaseImageByNameURL")
	}

	_basePath := o._basePath
	if _basePath == "" {
		_basePath = "/v1/baseimage"
	}
...
func (o *UpdateBaseImageByNameURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateBaseImageByNameURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateBaseImageByNameURL")
	}
