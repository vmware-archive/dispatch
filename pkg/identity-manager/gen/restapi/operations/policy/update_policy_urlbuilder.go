...
func (o *UpdatePolicyURL) Build() (*url.URL, error) {
	...
	var _path = "/v1/iam/policy/{policyName}"

	policyName := o.PolicyName
	if policyName != "" {
		_path = strings.Replace(_path, "{policyName}", policyName, -1)
	} else {
		return nil, errors.New("PolicyName is required on UpdatePolicyURL")
	}
...
func (o *UpdatePolicyURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdatePolicyURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdatePolicyURL")
	}
