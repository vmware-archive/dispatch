...
type Driver interface {
	...
	LockEntity(name string) (string, bool, error)
	ReleaseEntity(lock string) error
...
func (d *Zdriver) reConnect() error {
	...
	if err != nil {
		return errors.Wrapf(err, "Unable to reconnect to zookeeper after losing session")
	}
	return nil
}
...
func (d *Zdriver) LockEntity(entity string) (string, bool, error) {
	err := d.reConnect()
	if err != nil {
		return "", false, err
	}
	...
	if err != nil {
		return "", false, errors.Wrapf(err, "Unable to create lock node for %v", entity)
	}
	children, _, err := d.client.Children(fmt.Sprintf("/entities/%v", entity))
	if err != nil {
		return "", false, errors.Wrapf(err, "Unable to get children of entity %v", entity)
	}
...
func (d *Zdriver) ReleaseEntity(path string) error {
	err := d.reConnect()
	if err != nil {
		return err
	}
	...
	if err != nil {
		return errors.Wrapf(err, "Unable to delete lock %v", path)
	}
