...
// service includes the a planvailables
func (c *k8sServiceCatalogClient) ListServiceClasses() ([]entitystore.Entity, error) {

	cscs, err := c.sdk.RetrieveClasses(servicecatalog.ScopeOptions{Scope: servicecatalog.ClusterScope})
	if err != nil {
		return nil, errors.Wrapf(err, "Error retreiving service class list")
	}
	var serviceClasses []entitystore.Entity

	for _, class := range cscs {
		csc := class.(*v1beta1.ClusterServiceClass)
